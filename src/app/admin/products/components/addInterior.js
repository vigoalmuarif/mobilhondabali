...
export default function Interiors() {
  const [interiors, setInterios] = useState([]);
  function handleinteriors() {
    setInterios([...interiors, { id: nextID++ }]);
  }

  function handleTrash(id) {
    const data = interiors.filter((item) => {
      return item.id !== id;
    });
    setInterios(data);
  }
  return (
    <>
      ...
      <button type="button" className="btnSecondary mt-2" onClick={handleinteriors}>
        Tambah Interior
      </button>
    </>
  );
